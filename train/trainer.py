...
# from train.models import  OntoProteinKELoss, OntoProteinMLMLoss, E2EModel
# from kge_model.model import KGEModel
...
class Trainer(Trainer):
    ...
    def train(
        ...
        lm_scheduler = lambda step :( 1 + np.cos((step) * np.pi / max_steps) ) * 0.5
        ke_scheduler = lambda step :((max_steps - step) / float(max(1, max_steps )))
